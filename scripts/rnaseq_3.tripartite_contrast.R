...
#CL28 vs NB
res_cl28 <- results(object = dds,contrast = c("Syncom","CL28","NB")) %>%
  as.data.frame
inducted_genes_cl28 <- res_cl28 %>% subset(padj < alpha_cutoff & log2FoldChange > 0) %>% rownames %>%
  as.character

#CL28 vs CL28_CL14
res_duo <- results(object = dds,contrast = c("Syncom","CL28","CL28_CL14")) %>%
  as.data.frame

res_projected <- match(inducted_genes_cl28,rownames(res_duo)) %>% res_duo[.,] %>% droplevels
...
oh.save.pdf(p = p,outname = "rnaseq_contrast_dropout_inductedcl28_repressedcl28cl14.pdf",
            height = 20,width = 18,outdir = "../figures/")

#Save the plot
saveRDS(object = p,file = "../figures/rnaseq_contrast_dropout_inductedcl28_repressedcl28cl14.RDS")

#Save structures of expression and glm results
Dat <- list(dds = dds, Expression = mdf_all,
            chosen_genes = inducted_repressed)

saveRDS(object = Dat,file = "../cleandata/dataset_rnaseq_contrast_dropout_inductedcl28_repressedcl28cl14.RDS")
