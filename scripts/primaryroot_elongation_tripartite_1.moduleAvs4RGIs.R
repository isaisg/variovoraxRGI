...
Map_sub <- read.table(file = "../rawdata/dataS5_primaryroot_elongation_moduleAvs4RGIs.tsv",
                  header = T,sep = "\t")
#Relevel the NB
Map_sub$InhibitorStrain <- Map_sub$InhibitorStrain %>% factor %>% 
  relevel(ref = "NB")
#Test difference using anova
Res <- NULL
for(st in levels(Map_sub$InhibitedStrain)){
  Map_temp <- subset(Map_sub,InhibitedStrain == st)
  rdf <- aov(formula = MainRoot ~ InhibitorStrain ,data =Map_temp ) %>% emmeans(specs = "InhibitorStrain") %>%
...
Res$InhibitorStrain <- Res %$% contrast %>% gsub(pattern = "\\-NB",replacement = "") %>% factor
...
meta_chosen <- Map_sub$InhibitorStrain %>% levels %>%
...
cairo_pdf(filename = "../figures/primaryroot_elongation_tripartite_moduleAvs4RGIs_phylogeny.pdf",onefile = FALSE, fallback_resolution = 1200,
          width = 15, height = 1, family = "Arial", antialias = "default",
          pointsize = 12)

plot(tree,align.tip.label = T,show.tip.label = F,adj = 1,
     no.margin = TRUE,cex = 0.0001,direction = "downwards")
tiplabels(pch = 21,col = "#414141",
          bg = meta$ClassColor,cex = 1.5)
dev.off()
...
#
Map_sub$InhibitorStrain <- Map_sub %$% InhibitorStrain %>% factor(levels = c("NB",(meta$Freezer_Id)))
Map_sub$InhibitedStrain <- factor(Map_sub$InhibitedStrain,levels = c("MF48","MF107","MF224","CL28") %>% rev)
...
Map_sub$InhibitorStrain <- Map_sub$InhibitorStrain  %>% 
  factor(levels = Map_sub$InhibitorStrain %>% levels %>% rev)
p_tile <- ggplot(data = Map_sub,aes(x=InhibitorStrain,y=InhibitedStrain)) +
...
oh.save.pdf(p = p_tile,outname = "primaryroot_elongation_tripartite_moduleAvs4RGIs_heatmap_legends.pdf",
            outdir = "../figures/")
p_tile <- p_tile + theme(legend.position = "none")

oh.save.pdf(p = p_tile,outname = "primaryroot_elongation_tripartite_moduleAvs4RGIs_heatmap_nolegends.pdf",
            outdir = "../figures/",width = 25)

saveRDS(object = Map_sub,file = "../cleandata/dataset_tripartite_moduleAvs4RGIs.RDS")
